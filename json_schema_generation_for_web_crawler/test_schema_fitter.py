...
def test_fit():
    ...
    assert fit([1, 1.2, None]) == List(Union({Simple(int), Simple(float), Simple(None)}))
    assert fit([{'a': 1, 'b': 2}, {'a': 1, 'b': 5}]) == List(Dict({'a': Simple(int), 'b': Simple(int)}))
    assert fit([]) == List(Unknown())

def test_unify_dict_callback():
    assert fit({'1': 1, '2': 2}, unify_callback=try_unify_dict) == Dict({'1': Simple(int), '2': Simple(int)})
    assert fit(
