...
def main(pkg):
    ...
    if os.path.exists(log_path):
        ...
        with open('template.Dockerfile', 'r') as f:
            dockerfile = f.read().replace('{!python_package!}', pkg).replace('{!datetime!}', str(datetime.now()).replace(' ', 'T'))
        with open(f'dockerfiles/{pkg}.Dockerfile', 'w') as f:
            f.write(dockerfile)
        try:
            os.system(f'docker build -f dockerfiles/{pkg}.Dockerfile -t {pkg} . &> build_logs/{pkg}.log')
            image_id = os.popen(f"docker images -q '{pkg}' | uniq").readlines()[0].replace('\n', '')
            subprocess.run(f"docker rmi --force".split() + [image_id], check=True)
